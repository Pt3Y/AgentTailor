...
class Encoder:
    ...
    def __init__(self, model_name="all-MiniLM-L6-v2"):
        ...
        # Test connectivity before creating the model
        test_huggingface_connection(model_name)
        
        self.device = "cuda" if torch.cuda.is_available() else "cpu"
        
        print(f"📥 Start loading model: {model_name}")
        # Try loading the model with a retry mechanism
        max_retries = 5
        retry_delay = 2
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    print(f"⚠️  Load failed (attempt {attempt + 1}/{max_retries}): {error_msg[:100]}")
                    print(f"   🔄 Retrying in {retry_delay}s...")
                    time.sleep(retry_delay)
                    retry_delay = min(retry_delay * 2, 8)
                else:
                    print(f"\n❌ Model load failed after {max_retries} retries")
                    print(f"   Error details: {error_msg[:300]}")
                    print("\n💡 Troubleshooting:")
                    print("   1. Check network connectivity")
                    print("   2. Set environment variables manually:")
                    print("      Windows: set HF_MIRROR=https://hf-mirror.com")
                    print("      Linux/Mac: export HF_MIRROR=https://hf-mirror.com")
                    print("   3. Use a proxy/VPN if required")
                    raise
